...
interface HapusMasterTipeAsetProps {
...
const HapusMasterTipeAset: React.FC<HapusMasterTipeAsetProps> = ({ 
  tipe, 
  open, 
  onOpenChange, 
  onSuccess 
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/api/master-tipe-aset/${tipe.id_tipe}`);
      
      toast({
        title: 'Success',
        description: 'Master tipe aset berhasil dihapus',
      });

      onOpenChange(false);
      onSuccess();
    } catch (error: any) {
      const message = error.response?.data?.error || 'Gagal menghapus master tipe';
      toast({
        title: 'Error',
        description: message,
        variant: 'destructive',
      ...
      <AlertDialogContent className="sm:max-w-[425px] w-[95vw] max-h-[90vh] overflow-y-auto">
        <AlertDialogHeader>
          <AlertDialogTitle className="text-base sm:text-lg">
            Hapus Master Tipe Aset
          </AlertDialogTitle>
          <AlertDialogDescription className="text-xs sm:text-sm space-y-2 sm:space-y-3">
            <p>Apakah Anda yakin ingin menghapus tipe aset berikut?</p>
            ...
            <div className="space-y-1 text-xs sm:text-sm bg-muted/50 p-2 sm:p-3 rounded-lg">
              ...
              {tipe?.keterangan && (
                <div>
                  <strong>Keterangan:</strong> {tipe.keterangan}
                </div>
              )}
            ...
            <p className="text-xs text-muted-foreground">
              Semua master aset yang menggunakan tipe ini mungkin akan terpengaruh.
            </p>
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0">
          ...
          >
            {isLoading ? 'Menghapus...' : 'Hapus Tipe'}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
export default HapusMasterTipeAset;