...
interface ViewMasterMerekAsetProps {
  merek: MasterMerekAset | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const ViewMasterMerekAset: React.FC<ViewMasterMerekAsetProps> = ({ 
  merek, 
  open, 
  onOpenChange 
}) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] w-[95vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">Detail Master Merek Aset</DialogTitle>
          <DialogDescription className="text-xs sm:text-sm">
            Informasi lengkap tentang merek aset
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="grid grid-cols-1 gap-1 sm:gap-2">
              ...
              <div className="bg-muted/50 rounded-lg p-2 sm:p-3">
                <p className="text-xs sm:text-sm text-muted-foreground whitespace-pre-wrap">
                  {merek?.keterangan || 'Tidak ada keterangan'}
                </p>
              </div>
            </div>
          </div>
...
export default ViewMasterMerekAset;